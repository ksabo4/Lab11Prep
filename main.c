...
void insertionSort(int theArray[], int arraySize) {
  ...
  for (int pass = 1; pass < arraySize; pass++) {
    indexOfSmallest = pass;
    // Find smallest element among those still being sorted
    while ((indexOfSmallest > 0) &&
           (theArray[indexOfSmallest] < theArray[indexOfSmallest - 1])) {
...
{
  // To find index of middle do it the funky way shown below to avoid integer
  // overflow
